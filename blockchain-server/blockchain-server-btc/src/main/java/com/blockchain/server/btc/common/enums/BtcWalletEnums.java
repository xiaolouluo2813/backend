...
public enum BtcWalletEnums {
    ...
    NUMBER_INSUFFICIENT_GAS_ERROR(12700, "操作失败，提现手续费不足", "Operation failure, insufficient withdrawal charge",
            "操作失敗，提現手續費不足"),
    NUMBER_MINWDAMOUNT_ERROR(12700, "提现余额不得小于最小提现额度", "The withdrawal balance shall not be less than the minimum withdrawal limit", "提現余額不得小於最小提現額度"),
    // 数据的增删改报错的提示
    ...
    NUMBER_BLOCKED_ERROR(12701, "操作失败，输入金额必须大于0", "Operation failed, input number must be greater than 0", "操作失敗，输入金額必須大於0"),
    NUMBER_UNBLOCK_INSUFFICIENTZE_ERROR(12702, "操作失败，此前没有冻结过该账号余额，无法解冻", "The account balance has not been frozen before", "操作失敗，此前沒有凍結過該賬號余額，無法解凍"),
